...
// types
import { TChatConversation } from 'src/types/chat';
import useGetNavItem from '../hooks/use-get-nav-item';
...
export default function ChatNavItem({
  ...
  const { name, imageName, status } = singleParticipant;
    ...
    >
      <AvatarGroup variant="compact" sx={{ width: 48, height: 48 }}>
        {participants.slice(0, 2).map((participant) => (
          <Avatar key={participant.id} alt={participant.name} src={participant.imageName} />
        ))}
      </AvatarGroup>
    </Badge>
  );
  ...
  const renderSingle = (
    <Badge key={status} variant={status} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}>
      <Avatar alt={name} src={imageName} sx={{ width: 48, height: 48 }} />
    </Badge>
  );
