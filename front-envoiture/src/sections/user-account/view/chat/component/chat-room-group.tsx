...
export default function ChatRoomGroup({ participants }: Props) {
  const [selected, setSelected] = useState<TChatParticipant | null>(null);

  const collapse = useBoolean(true);

  const handleOpen = useCallback((participant: TChatParticipant) => {
    ...
    <Scrollbar sx={{ height: 56 * 4 }}>
      {participants.map((participant) => (
        <ListItemButton key={participant.id} onClick={() => handleOpen(participant)}>
          ...
          >
            <Avatar alt={participant.name} src={participant.imageName} />
          </Badge>

          <ListItemText
            sx={{ ml: 2 }}
            primary={participant.name}
            secondary={<Typography>{participant.age}</Typography>}
